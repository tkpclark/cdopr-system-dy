...
static int fetch_data()
{
	...
	sprintf(sql,"select * from wraith_message where motime > NOW()-interval 1 hour and mo_status='ok' and gwid=%s and gw_resp is null and mt_message is not null and mt_message!='' limit 500", gwid);
	//sprintf(sql,"select * from wraith_message where ID > %d and mo_status='ok' and gwid=%s limit 500",0, gwid);
	proclog(sql);
