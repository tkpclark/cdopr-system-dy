...
def f_mr(record,mrurl):
    ...
    #time.sleep(1)
    nowtime = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
    report='0' if record['report']=='1' else '1'
    url = '%s?msgtype=0&spnumber=%s&msgcontent=%s&msisdn=%s&linkid=%s&msgreport=%s&createtime=%s' \
...
def f_mo_qishun(record,mourl):
    #logging.info('forwarding record %s',record)
    #time.sleep(1)
    #http://youraddress/interface_mo?spnumber=106673336&msg=CP&fee=2&mobile=13179386983&linkid=72523970&createtime=20120320095009
    nowtime = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
    url = '%s?forward_type=mo&sp_number=%s&message=%s&phone_number=%s&linkid=%s' \
    %(mourl,record['sp_number'],record['mo_message'],record['phone_number'],record['linkid'])
    logging.info('(%s):%s',record['id'], url)
    forward_result,forward_resp = visit_url(url)
    forward_status = 1 if(forward_result == 1) else 6 #1：转发成功 6：mo转发失败
    return(forward_status,forward_result,forward_resp,url) 
    
def f_mr_qishun(record,mrurl):
    #logging.info('forwarding record %s',record)
    #time.sleep(1)
    nowtime = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
    report='OK' if record['report']=='1' else record['report']
    url = '%s?forward_type=mr&sp_number=%s&message=%s&phone_number=%s&linkid=%s&report=%s' \
    %(mrurl,record['sp_number'],record['mo_message'],record['phone_number'],record['linkid'],report)
    logging.info('(%s):%s',record['id'], url)
    forward_result,forward_resp = visit_url(url)
    forward_status = 2 if(forward_result == 1) else 7 
    return(forward_status,forward_result,forward_resp,url) 
